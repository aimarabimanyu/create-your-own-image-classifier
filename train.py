...
def model_classifier(args, output_shape_pretrained_model, class_to_idx):
    # Define fully-connected layer from pretrained model to suit our problem
    model_classifier = nn.Sequential(nn.Linear(output_shape_pretrained_model, args.hidden_units),
                                     nn.ReLU(),
                                     nn.Dropout(0.2),
...
def model_arch(args, class_to_idx):
    ...
    if args.arch in ['vgg11', 'vgg13', 'vgg16']:
        ...
        output_shape_pretrained_model = model.classifier[0].in_features

        # Define fully-connected layer from pretrained model to suit our problem
        model.classifier = model_classifier(args, output_shape_pretrained_model, class_to_idx)
        ...
        # Define fully-connected layer from pretrained model to suit our problem
        model.fc = model_classifier(args, output_shape_pretrained_model, class_to_idx)
...
def model_training(args, model, criterion, optimizer, train_dataloaders, val_dataloaders):
    ...
    if args.gpu and torch.cuda.is_available():
        device = 'cuda:0'
        print('GPU available, model train with CUDA')
    else:
        device = 'cpu'
        print('GPU unavailable, Model train with CPU')
...
def model_saving(args, model, optimizer, class_to_idx):
    ...
    checkpoint = {
        'classifier' : model.classifier if args.arch in ['vgg11', 'vgg13', 'vgg16'] else model.fc,
...
def main():
    ...
    parser.add_argument('--arch', dest='arch', default='resnet50', choices=['resnet18', 'resnet34', 'resnet50', 'resnet101', 'vgg11', 'vgg13', 'vgg16'])
    parser.add_argument('--learning_rate', dest='learning_rate', default=0.001, type=float)
    parser.add_argument('--hidden_units', dest='hidden_units', default=128, type=int)
    ...
    model_saving(args, model, optimizer, class_to_idx)
